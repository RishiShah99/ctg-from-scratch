...
```
esp32/
├── platformio.ini             ← board + build config (uses -I../src to pull in the shared kernel header)
└── src/
...
The forward kernel header is the single file at `../src/osdn_inference.h`,
included by both this firmware and the host bit-identity test.
